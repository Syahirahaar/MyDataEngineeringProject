...
def lambda_handler(event, context):
    ...
    for record in event['Records']:
        ...
        customer_key.update({'CustomerID': {"N": str(dict_record['CustomerID'])}, 'timestamp' : {"S": str(dict_record['timestamp'])},'flight_id':{"S":str(dict_record['flight_id'])}})




        ex_customer = dict()
        ex_customer.update({str(dict_record['flight']): {'Value':{"S":'Some overview JSON for the UI goes here'},"Action":"PUT"}})
